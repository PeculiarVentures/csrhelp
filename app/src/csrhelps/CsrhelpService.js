(function(){
    ...
    * @constructor
    */
    function CsrhelpService($q, $timeout){
    // Promise-based API
        ...
        function make_keytool(certf, context) {
            var keytool='keytool -genkey -alias server -keyalg '+certf.algorithm+
            ' -sigalg SHA256withRSA -keysize '+certf.keysize+
            ' -keystore '+getFilename(certf.hostname)+'.jks -dname "';

            if(certf.algorithm == "ECC")
                keytool='keytool -genkeypair -keyalg EC -sigalg SHA256withECDSA '+
            '-keysize '+getECCKeysize('keytool',certf.keysize)+
            ' -keystore '+getFilename(certf.hostname)+
            '.jks -dname "';
        ...
        function make_iis(certf, context) {
            ...
            if(temp) {
                if(certf.algorithm =="ECC"){
                    ...
                    'echo KeyAlgorithm="'+getECCKeysize("iis", certf.keysize)+'" >>csrparams.inf\r\n'+
