  ...
  function CsrhelpController( csrhelpService, $mdSidenav, $mdBottomSheet, $log, $q, $scope) {
	    ...
	    $scope.checkToGenerate = function() {
	    		...
	    		if($scope.certificate.keysize == 4096 && $scope.certificate.algorithm =="ECC") {
	    				$scope.certificate.algorithm = "";
	    		}
	    ...
	    $scope.make_openssl = function() {
	    		var openssl='openssl req -new -newkey rsa:'+ $scope.certificate.keysize +' -nodes -out '+$scope.getFilename($scope.certificate.hostname)+'.csr -keyout '+$scope.getFilename($scope.certificate.hostname)+'.key -subj "';
					...
					if($scope.certificate.algorithm == "ECC") {
						openssl='openssl ecparam -name secp256r1 - genkey -noout -out '+$scope.getFilename($scope.certificate.hostname)+'.key\r\n';
						openssl+='openssl req -new -out '+$scope.getFilename($scope.certificate.hostname)+'.csr -key '+$scope.getFilename($scope.certificate.hostname)+'.key -subj "';
						if($scope.certificate.country)
							openssl+='/C='+$scope.certificate.country;
						if($scope.certificate.state)				
							openssl+='/ST='+$scope.certificate.state;
	    ...
	    $scope.make_keytool = function() {
	    		var keytool='keytool -genkey -alias server -keyalg '+$scope.certificate.algorithm+' -keysize '+$scope.certificate.keysize+' -keystore '+$scope.getFilename($scope.certificate.hostname)+'.jks -dname "';
					var temp=false;
	    ...
	    $scope.make_bigip = function() {
	    		var bigip=' create sys crypto key '+$scope.getFilename($scope.certificate.hostname)+' key-size '+$scope.certificate.keysize+' gen-csr';
	    ...
	    $scope.getFilename = function(str) {
			...
			$scope.copyToClipboard = function($event, item) {
				    if (window.getSelection && document.createRange) {
				        ...
				        textRange.moveToElementText($($event.currentTarget).parent().find("pre"));
				        textRange.select();
				    }
				
			}
		$scope.hostname_change = function() {
			if($("#hostname").val()==".example.com")
				$scope.certificate.hostname = "star.example.com";
