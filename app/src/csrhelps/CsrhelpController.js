  ...
  function CsrhelpController( csrhelpService, $mdSidenav, $mdBottomSheet, $log, $q, $scope) {
	    ...
	    $scope.make_openssl = function() {
	    		var openssl='openssl req -new -newkey rsa:'+ $scope.certificate.keysize +' -sha256 -nodes -out '+$scope.getFilename($scope.certificate.hostname)+'.csr -keyout '+$scope.getFilename($scope.certificate.hostname)+'.key -subj "';
	    ...
	    $scope.make_exchange2007 = function() {	    	
	    		var exchange='New-ExchangeCertificate -GenerateRequest -Path .\\'+$scope.getFilename($scope.certificate.hostname)+'.csr -KeySize '+$scope.certificate.keysize+' -SubjectName "';
			var temp=false;
			...
			if(temp)
				exchange+=',cn='+$scope.certificate.hostname+'" -PrivateKeyExportable $True';
			else
				exchange+='cn='+$scope.certificate.hostname+'" -PrivateKeyExportable $True';
	    ...
	    $scope.make_exchange2010 = function() {
	    		var exchange='Set-Content -path ".\\'+$scope.getFilename($scope.certificate.hostname)+'.csr" -Value (New-ExchangeCertificate -GenerateRequest -KeySize '+$scope.certificate.keysize+' -SubjectName "';		
			var temp=false;
	    ...
	    $scope.make_keytool = function() {
	    		var keytool='keytool -genkey -alias server -keyalg '+$scope.certificate.algorithm+' -sigalg SHA256withRSA -keysize '+$scope.certificate.keysize+' -keystore '+$scope.getFilename($scope.certificate.hostname)+'.jks -dname "';	    		
	    		if($scope.certificate.algorithm == "ECC")
	    			keytool='keytool -genkeypair -keyalg EC -sigalg SHA256withECDSA -keysize '+$scope.getECCKeysize('keytool',$scope.certificate.keysize)+' -keystore '+$scope.getFilename($scope.certificate.hostname)+'.jks -dname "';
			var temp=false;
			...
			if(temp) {
				if($scope.certificate.algorithm == "ECC")
					keytool+=',CN='+$scope.certificate.hostname+'" -alias server && keytool -certreq -alias server -file '+$scope.getFilename($scope.certificate.hostname)+'.csr -keystore '+$scope.getFilename($scope.certificate.hostname)+'.jks';
				else
					keytool+=',CN='+$scope.certificate.hostname+'" && keytool -certreq -alias server -file '+$scope.getFilename($scope.certificate.hostname)+'.csr -keystore '+$scope.getFilename($scope.certificate.hostname)+'.jks';
			}
			else {
				if($scope.certificate.algorithm == "ECC")
					keytool+='CN='+$scope.certificate.hostname+'" -alias server && keytool -certreq -alias server -file '+$scope.getFilename($scope.certificate.hostname)+'.csr -keystore '+$scope.getFilename($scope.certificate.hostname)+'.jks';
	    ...
	    $scope.make_bigip = function() {
	    		var bigip='create sys crypto key '+$scope.certificate.hostname+' key-size '+$scope.certificate.keysize+' gen-csr';
			...
			bigip+=' common-name "'+$scope.certificate.hostname+ '"';
			if($scope.certificate.algorithm == "ECC") {
				var bigip='create key '+$scope.getFilename($scope.certificate.hostname)+' key-type ec-private curve-name '+$scope.getECCKeysize('bigip', $scope.certificate.keysize)+' gen-csr';
	    ...
	    $scope.make_any = function() {
	            ...
	            pkcs10_simpl.subject.types_and_values.push(new org.pkijs.simpl.ATTR_TYPE_AND_VALUE({ type: "2.5.4.3", value: new org.pkijs.asn1.UTF8STRING({ value: $scope.certificate.hostname }) }));
		...
		$scope.download = function($event, item) {
			if($scope.certificate.hostname=="*.example.com")
	                ...
	                //console.log('data:application/pkcs8;base64,'+encodeURIComponent($scope.messages[6].privateKey));
			$($event.currentTarget).attr('href','data:application/pkcs8;base64,'+encodeURIComponent($scope.messages[6].privateKey)).attr('download', $scope.certificate.filehostname+'.key');
